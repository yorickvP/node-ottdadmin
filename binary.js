...
// based on https://github.com/substack/node-bufferlist (hacked in zstring)
...
function Binary(buffer) {
    ...
    this.zstring = function () {
        var into = [].reduce.call(arguments, function (acc,x) {
            return acc.concat(x);
        }, []);
        this.pushAction({
            ready : function () {
                return [].indexOf.call(buffer.join(this.offset), 0) != -1;
            },
            action : function () {
                var len = [].indexOf.call(buffer.join(this.offset), 0);
                var data = buffer.join(this.offset, this.offset + len);
                this.offset += len + 1;
